...
export default function VotePage() {
  ...
  async function chargerVotes1() {
    ...
    data?.forEach(v => {
      ...
      if (v.joueur_id === joueurId) {
        mesCounts[v.game_id] = true
      }
    })
  ...
  async function chargerVotes2() {
    const { data } = await supabase
      .from('votes_phase2')
      .select('game_id, nombre')
      .eq('room_code', code)

    const counts = {}

    data?.forEach(v => {
      counts[v.game_id] = (counts[v.game_id] || 0) + v.nombre
    })
  ...
  async function toggleVote1(gameId) {
    ...
    if (dejaVote) {
      ...
      setVotes1(prev => ({
        ...prev,
        [gameId]: Math.max((prev[gameId] || 1) - 1, 0)
      }))
